...
function Round4({baseURL, questions}){
  ...
  function tts(text){
    ...
    useEffect(()=>{
        ...
        setCurrent(index < questions.length&&<QuestionSimulationRound4 question={questions[index]["Question"]} baseURL={baseURL} sub={questions[index]["Subject"]} questionId={questions[index]["id"]} title={false} next={setNextValid} scoring={setScore} save={false} key={index}/>|| "")
        if(index == questions.length){
          http.post(`${baseURL}/save_round`, { round:"Round 4", score:score, questions:questions.length })
         }
      },[index])
    return (index < questions.length&&<div className="p-5">
        <h1>Practice Round 4, Question No. {index+1} out of {questions.length}, Score: {score}</h1>
        {current}

        {index < questions.length && nextValid && <button onClick={()=>{setIndex((cur)=>cur+1); setNextValid(false)}}>Next Question</button>}
    </div>||<div className="p-5">
        <h1>Result: Practice Round 4</h1>
